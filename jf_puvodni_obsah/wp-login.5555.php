...
function retrieve_password() {
	...
	$message .= '<' . network_site_url("wp-login.5555.php?action=rp&key=$key&login=" . rawurlencode($user_login), 'login') . ">\r\n";
...
case 'logout' :
	...
	$redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'wp-login.5555.php?loggedout=true';
...
case 'retrievepassword' :
	...
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			$redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'wp-login.5555.php?checkemail=confirm';
			wp_safe_redirect( $redirect_to );
			exit();
		}
	}
...
<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( site_url( 'wp-login.5555.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
	<p>
...
<?php if ( get_option( 'users_can_register' ) ) : ?>
 | <a href="<?php echo esc_url( site_url( 'wp-login.5555.php?action=register', 'login' ) ); ?>"><?php _e( 'Register' ); ?></a>
<?php endif; ?>
</p>
...
case 'rp' :
	...
	if ( is_wp_error($user) ) {
		wp_redirect( site_url('wp-login.5555.php?action=lostpassword&error=invalidkey') );
		exit;
	}
...
?>
<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( site_url( 'wp-login.5555.php?action=resetpass&key=' . urlencode( $_GET['key'] ) . '&login=' . urlencode( $_GET['login'] ), 'login_post' ) ); ?>" method="post">
...
case 'register' :
	...
	if ( !get_option('users_can_register') ) {
		wp_redirect( site_url('wp-login.5555.php?registration=disabled') );
		exit();
	}
	...
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			$redirect_to = !empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : 'wp-login.5555.php?checkemail=registered';
			wp_safe_redirect( $redirect_to );
			exit();
		}
	}
...
<form name="registerform" id="registerform" action="<?php echo esc_url( site_url('wp-login.5555.php?action=register', 'login_post') ); ?>" method="post">
	<p>
...
<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'wp-login.5555.php', 'login_post' ) ); ?>" method="post">
	<p>
		<label for="user_login"><?php _e('Username') ?><br />
