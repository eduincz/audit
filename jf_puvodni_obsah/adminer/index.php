...
confirm($Za="",$Ze=false){return" onclick=\"".($Ze?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($Za?" (' + $Za + ')":"")."');\"";}function
...
upload_error($k){$cd=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($k?'Nepodařilo se nahrát soubor.'.($cd?" ".sprintf('Maximální povolená velikost souboru je %sB.',$cd):""):'Soubor neexistuje.');}function
...
enum_input($S,$xa,$l,$X,$Bb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$Xc);$G=(isset($Bb)?"<label><input type='$S'$xa value='$Bb'".((is_array($X)?in_array($Bb,$X):$X===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($Xc[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ja=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$G.=" <label><input type='$S'$xa value='".($q+1)."'".($Ja?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
input($l,$X,$o){global$T,$b,$v;$A=h(bracket_escape($l["field"]));echo"<td class='function'>";$_e=($v=="mssql"&&$l["auto_increment"]);if($_e&&!$_POST["save"]){$o=null;}$jc=(isset($_GET["select"])||$_e?array("orig"=>'původní'):array())+$b->editFunctions($l);$xa=" name='fields[$A]'";if($l["type"]=="enum"){echo
...
q($W);}function
check_sqlite_name($A){global$g;$Tb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Tb)\$~",$A)){$g->error=sprintf('Prosím použijte jednu z koncovek %s.',str_replace("|",", ",$Tb));return
false;}return
true;}function
...
set_schema($Je){global$g,$T,$bf;$G=$g->query("SET search_path TO ".idf_escape($Je));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$bf['Uživatelské typy'][]=$S;}}return$G;}function
...
reset($I);}function
support($Vb){return
ereg("view|drop_col|variables|status",$Vb);}$v="oracle";$T=array();$bf=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Řetězce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binární'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$bf[$w]=array_keys($W);}$Pf=array();$Cd=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$jc=array("length","lower","round","upper");$nc=array("avg","count","count distinct","max","min","sum");$xb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$rb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$ee=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($if,$L=""){echo'<p class="tabs">';$Sc=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($if)){$Sc["view"]='Pozměnit pohled';}else{$Sc["create"]='Pozměnit tabulku';}if(isset($L)){$Sc["edit"]='Nová položka';}foreach($Sc
...
selectColumnsPrint($J,$f){global$jc,$nc;print_fieldset("select",'Vypsat',$J);$q=0;$ic=array('Funkce'=>$jc,'Agregace'=>$nc);foreach($J
...
false;}function
messageQuery($E){global$v;static$Za=0;restart_session();$r="sql-".($Za++);$pc=&get_session("queries");if(strlen($E)>1e6){$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";}$pc[$_GET["db"]][]=$E;return" <a href='#$r' onclick=\"return !toggle('$r');\">".'SQL příkaz'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($pc[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
';if($ld!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$ld){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvořit novou tabulku'."</a>\n";$Q=tables_list();if(!$Q){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($Q);$Sc=array();foreach($Q
...
edit_type($w,$l,$Oa,$dc=array()){global$bf,$T,$Pf,$zd;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$bf+($dc?array('Cizí klíče'=>$dc):array()),$l["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($Oa,$l["collation"]).'</select>',($Pf?"<select name='$w"."[unsigned]'".(!$l["type"]||ereg('(int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($Pf,$l["unsigned"]).'</select>':''),($dc?"<select name='$w"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist(explode("|",$zd),$l["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Ta?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($Ta?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($l["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $va, 1);'>&nbsp;"."<input type='image' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.3' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.3' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($Jd==""||support("drop_col")?"<input type='image' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.3' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$w=>$W){echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'Změnit'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Přidat trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databáze',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$lf=array();$mf=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Xc,PREG_SET_ORDER);foreach($Xc
...
as$A=>$l){if(!isset($l["privileges"][$Qf?"update":"insert"])||$b->fieldName($l)==""){unset($m[$A]);}}if($_POST&&!$k&&!isset($_GET["select"])){$Tc=$_POST["referer"];if($_POST["insert"]){$Tc=($Qf?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$Tc)){$Tc=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$Tc,'Položka byla smazána.');}else{$L=array();foreach($m
as$A=>$l){$W=process_input($l);if($W!==false&&$W!==null){$L[idf_escape($A)]=($Qf?"\n".idf_escape($A)." = $W":$W);}}if($Qf){if(!$L){redirect($Tc);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$Tc,'Položka byla aktualizována.');}else{$F=insert_into($a,$L);$Mc=($F?last_id():0);queries_redirect($Tc,sprintf('Položka%s byla vložena.',($Mc?" $Mc":"")),$F);}}}$jf=$b->tableName(table_status($a));page_header(($Qf?'Upravit':'Vložit'),$k,array("select"=>array($a,$jf)),$jf);$H=null;if($_POST["save"]){$H=(array)$_POST["fields"];}elseif($Z){$J=array();foreach($m
...
';if($m){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Qf?'Uložit a pokračovat v editaci':'Uložit a vložit další')."' title='Ctrl+Shift+Enter'>\n";}}echo($Qf?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$Ud=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Rozdělit podle',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Td,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Oddíly: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($Ud||!$H["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$k&&!isset($_POST["add_x"])){restart_session();if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáze byla odstraněna.',drop_databases(array(DB)));}elseif(DB!==$_POST["name"]){if(DB!=""){$_GET["db"]=$_POST["name"];queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($_POST["name"]),'Databáze byla přejmenována.',rename_database($_POST["name"],$_POST["collation"]));}else{$i=explode("\n",str_replace("\r","",$_POST["name"]));$cf=true;$Lc="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$_POST["collation"])){$cf=false;}$Lc=$j;}}queries_redirect(ME."db=".urlencode($Lc),'Databáze byla vytvořena.',$cf);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$k,array(),DB);$Oa=collations();$A=DB;$Na=null;if($_POST){$A=$_POST["name"];$Na=$_POST["collation"];}elseif(DB!=""){$Na=db_collation(DB,$Oa);}elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<p>
Při smazání: ',html_select("on_delete",array(-1=>"")+explode("|",$zd),$H["on_delete"]),' Při změně: ',html_select("on_update",array(-1=>"")+explode("|",$zd),$H["on_update"]),'<p>
<input type="submit" value="Uložit">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$tb=false;if($_POST&&!$k){$tb=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($_POST["name"])." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_POST["name"])),'Pohled byl odstraněn.','Pohled byl změněn.','Pohled byl vytvořen.',$a);}page_header(($a!=""?'Pozměnit pohled':'Vytvořit pohled'),$k,array("table"=>$a),$a);$H=$_POST;if(!$H&&$a!=""){$H=view($a);$H["name"]=$a;}echo'
<form action="" method="post">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Cc,$H["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Ye,$H["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložit">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($tb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$k){$z=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sekvence byla odstraněna.');}elseif($fa==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$z,'Sekvence byla vytvořena.');}elseif($fa!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($_POST["name"]),$z,'Sekvence byla změněna.');}else{redirect($z);}}page_header($fa!=""?'Pozměnit sekvenci'.": ".h($fa):'Vytvořit sekvenci',$k);$H=$_POST;if(!$H){$H=array("name"=>$fa);}echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Uložit">
';if($fa!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$k){$z=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($ga),$z,'Typ byl odstraněn.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$z,'Typ byl vytvořen.');}}page_header($ga!=""?'Pozměnit typ'.": ".h($ga):'Vytvořit typ',$k);$H=$_POST;if(!$H){$H=array("as"=>"AS ");}echo'
<form action="" method="post">
<p>
...
as$W){if($W[1]!="USAGE"){$kc["$_[2]$W[2]"][$W[1]]=true;}if(ereg(' WITH GRANT OPTION',$H[0])){$kc["$_[2]$W[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$H[0],$_)){$wd=$_[1];}}}if($_POST&&!$k){$xd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");$rd=q($_POST["user"])."@".q($_POST["host"]);$Xd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $xd",ME."privileges=",'Uživatel byl odstraněn.');}else{$cb=false;if($xd!=$rd){$cb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $rd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Xd");$k=!$cb;}elseif($_POST["pass"]!=$wd||!$_POST["hashed"]){queries("SET PASSWORD FOR $rd = ".($_POST["hashed"]?$Xd:"PASSWORD($Xd)"));}if(!$k){$Ae=array();foreach($qd
...
as$ud=>$Ae){if(preg_match('~^(.+)(\\(.*\\))?$~U',$ud,$_)){grant("REVOKE",array_keys($Ae),$_[2]," ON $_[1] FROM $rd");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Uživatel byl změněn.':'Uživatel byl vytvořen.'),!$k);if($cb){$g->query("DROP USER $rd");}}}page_header((isset($_GET["host"])?'Uživatel'.": ".h("$ha@$_GET[host]"):'Vytvořit uživatele'),$k,array("privileges"=>array('','Oprávnění')));if($_POST){$H=$_POST;$kc=$qd;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$wd;if($wd!=""){$H["hashed"]=true;}$kc[DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")).".*":""]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$q=0;foreach($kc
as$ud=>$p){echo'<th>'.($ud!="*.*"?"<input name='objects[$q]' value='".h($ud)."' size='10'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Xa=>$mb){foreach((array)$le[$Xa]as$ke=>$Sa){echo"<tr".odd()."><td".($mb?">$mb<td":" colspan='2'").' lang="en" title="'.h($Sa).'">'.h($ke);$q=0;foreach($kc
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$Ic=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W))){$Ic++;}}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$Ic),$Ic||!$_POST["kill"]);}page_header('Seznam procesů',$k);echo'
<form action="" method="post">
