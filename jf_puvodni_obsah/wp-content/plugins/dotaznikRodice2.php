...
Description: Pridava dotaznik do stranky, ktera ma v custom fields klic dotaznikRodiceVitani, rozsireni o materske skoly 
...
*/
   ...
   if(isset($data['skola'][33]))$materska_skola=$data['skola'][33];
   elseif(get_post_custom_values('dotaznik_materska_skola', get_the_id()))$materska_skola=1;
   else $materska_skola=0;
   // if(isSpravaSkoly()){echo $materska_skola;print_r($data['skola']);}
...
if($_POST['akce']=='kontrola')
 ...
 {
 $sql="SELECT id FROM skoly WHERE ic_skoly='{$_POST['icSkoly']}' AND materska_skola='$materska_skola'";
 if(db_dotaz($sql) and !isSpravaSkoly()){echo "<p><strong style='color:red'>Škola s tímto IČ je již zaregistrována</strong>. Pokud si přejete svojí škole obnovit platnost certifikace, vyčkejte, dokud vám nedorazí automatický email s pokyny k obnovení certifikace. Pokud vám email nedorazil, obraťte se prosím na email <a href=\"mailto:libor.koci@eduin.cz\">libor.koci@eduin.cz</a></p>";return false;}
...
if($_POST['nazev'] and $_POST['adresa'] and $_POST['icSkoly'] and $_POST['telefon'] and $_POST['email'] and !isset($_GET['archiv']))
{

$sql="

UPDATE odpovedi_dotaznik SET `a1`= '$a1', `a2`='$a2' , `a3`='$a3' , `a4`='$a4' , `a5`='$a5' , `a6`='$a6' , `a7`='$a7' , `b1`='{$_POST['b1']}' , `b2`='{$_POST['b2']}' , `b3`='{$_POST['b3']}' , `b4`='{$_POST['b4']}' , `b5`='{$_POST['b5']}' , `b6`='{$_POST['b6']}' , `b7`='{$_POST['b7']}' , `b8`='{$_POST['b8']}' , `b9`='{$_POST['b9']}' , `b10`='{$_POST['b10']}' , `b11`='{$_POST['b11']}' , `b12`='{$_POST['b12']}' , `b13`='{$_POST['b13']}' , `b14`='{$_POST['b14']}' , `b15`='{$_POST['b15']}' , `b16`='{$_POST['b16']}' , `b17`='{$_POST['b17']}', ma22='{$_POST['metakriteria']['ma22']}',mb3='{$_POST['metakriteria']['mb3']}',mb6='{$_POST['metakriteria']['mb6']}',mb8='{$_POST['metakriteria']['mb8']}',mb12='{$_POST['metakriteria']['mb12']}',mb13='{$_POST['metakriteria']['mb13']}'
...
$sql="INSERT INTO `odpovedi_dotaznik` ( `id_skoly` , `a1` , `a2` , `a3` , `a4` , `a5` , `a6` , `a7` , `b1` , `b2` , `b3` , `b4` , `b5` , `b6` , `b7` , `b8` , `b9` , `b10` , `b11` , `b12` , `b13` , `b14` , `b15` , `b16` , `b17`,`varianta`,`ma22`,`mb3`,`mb6`,`mb8`,`mb12`,`mb13`)

VALUES ('$id_skoly', '$a1', '$a2', '$a3', '$a4', '$a5', '$a6', '$a7', '{$_POST['b1']}', '{$_POST['b2']}', '{$_POST['b3']}', '{$_POST['b4']}', '{$_POST['b5']}', '{$_POST['b6']}', '{$_POST['b7']}', '{$_POST['b8']}', '{$_POST['b9']}', '{$_POST['b10']}', '{$_POST['b11']}', '{$_POST['b12']}', '{$_POST['b13']}', '{$_POST['b14']}', '{$_POST['b15']}', '{$_POST['b16']}', '{$_POST['b17']}','".AKTUALNI_VARIANTA."','{$_POST['metakriteria']['ma22']}','{$_POST['metakriteria']['mb3']}','{$_POST['metakriteria']['mb6']}','{$_POST['metakriteria']['mb8']}','{$_POST['metakriteria']['mb12']}','{$_POST['metakriteria']['mb13']}');";
...
if($a1 && $a2 && $a3 && $a4 && $a5 && $a6 && $a7 && $volitelne>=2){ $splneno=true;}else $splneno=false;
  ...
  if(!$materska_skola  && $varianta==3){   
     if(count($_POST['a4'])==2) $splneno=true; else $splneno=false; //vyjimka pro variantu 3
    }
if($splneno) {                                                   
...
if(isSpravaSkoly())
$sql="UPDATE skoly SET geoLat='{$souradnice['geoLat']}',geoLng='{$souradnice['geoLng']}' WHERE id='$id_skoly'";
else
...
if(isSpravaSkoly())
{
$sql="UPDATE skoly SET certifikovana='1' WHERE id='$id_skoly' AND certifikovana='0'";//nastavit skolu na certifikovanou, kdyz ji administrator nasledne doplni splnene podminky
db_dotaz($sql);
}
...
if($_GET['page']=='recertifikace')
{
$sql="SELECT datumZverejneni,faze FROM skoly WHERE id='$id_skoly'";
$datum=db_dotaz($sql);
if($datum[0][1]=='proslaCertifikace')
$newDate = date("Y-m-d");//pokud je certifikace prosla, nastavit datum nove certifikace na dnesni
else
$newDate = date("Y-m-d",strtotime(date("Y-m-d", strtotime($datum[0][0])) . " +1 year"));//jinak nastavit datum o rok vyssi nez puvodni

$sql="UPDATE skoly SET datumZverejneni='$newDate',zobrazeno='1',faze='recertifikaceKeSchvaleni',certifikovana='1' WHERE id='$id_skoly'";
db_dotaz($sql);
echo "<div class='info'>
<p>Gratulujeme!
...
<p>
Vámi vyplněné údaje jsou uvedeny na konci emailu.
</p>
<p>
...
EDUin, o.p.s.<br />
+420 777 222 081<br />
libor.koci@eduin.cz<br />
www.rodicevitani.cz<br />
</p>
<p>
Vámi vyplněné údaje jsou následující:
".vyplnenoVeFormulariCertifikace($id_skoly)."
</p>
";
...
wp_mail('jana.spoutilova@eduin.cz', "úspěšná Recertifikace školy {$_POST['nazev']}", $teloAminMailu, $headers);
...
wp_mail('marek.drahovzal@seznam.cz', "úspěšná certifikace školy {$_POST['nazev']}", $teloAminMailu, $headers);
...
{ 
  ...
  {  
    ...
    wp_mail('marek.drahovzal@seznam.cz', "NEúspěšné obnovení certifikace školy {$_POST['nazev']}", $teloAminMailu, $headers);
...

  {
    ...
    wp_mail('marek.drahovzal@seznam.cz', "NEúspěšná certifikace školy {$_POST['nazev']}", $teloAminMailu, $headers);
...
$metakriteria=$data['metakriteria'];
...
if(!isset($_GET['archiv']))$res.='<br /><input type="submit" value="Uložit" />';
//$res.="</form>";
$res.="</div>";

}

 if($materska_skola) $sql="SELECT * FROM komentare_dotaznik_materska";
 else $sql="SELECT * FROM komentare_dotaznik";
...
foreach($komentare as $k) $komentare_kriterii[$k[0]]=$k[1];
  ...
  if($materska_skola)
  $sql="SELECT id,veta FROM vety_dotaznik_materska WHERE varianta='$varianta'";
  else $sql="SELECT id,veta FROM vety_dotaznik WHERE varianta='$varianta'";
...
$res.="
<script>
function display_metakriteria_box(check,id)
{
if($(check).is(':checked'))$('#'+id).show();else $('#'+id).hide();
}
</script>
";
...
<h3><span class='odrazka'>02</span>".komentar_kriteria($komentare_kriterii['a2'])." {$vety['pk2']} *</h3>";
    ...
    if(!empty($a2) and @in_array('v5',$a2))$sel="checked=\"checked\""; else $sel="";$res.="<input type='checkbox' name='a2[]' id='a22' value='v5' onchange='display_metakriteria_box(this,\"meta_a23\")' $sel/> 
    <label for='a22'>{$vety['v5']}</label><br />";

    if(!empty($metakriteria) and isset($metakriteria['ma22']))$val=$metakriteria['ma22']; else $val="";
    $res.="<div id='meta_a23' ".(!$sel?"style='display:none'":"").">
    <label for='a23' class='metakriteria'>{$vety['ma22']}</label><br />
    <textarea name='metakriteria[ma22]' id='a23'>$val</textarea><br />
    </div>";


 $res.="

<h3><span class='odrazka'>03</span>".komentar_kriteria($komentare_kriterii['a3'])." {$vety['pk3']} *</h3>";
...
$res.="<h3><span class='odrazka'>04</span>".komentar_kriteria($komentare_kriterii['a4'])." {$vety['pk4']} *</h3>";
...
if(isset($data['varianta']) and ($data['varianta']==1 or $data['varianta']==2)){//zmena struktury dotazniku od varianty 3
$res.="<h3><span class='odrazka'>05</span>".komentar_kriteria($komentare_kriterii['a5'])." {$vety['pk5']} *</h3>";
...
$res.="<h3><span class='odrazka'>06</span>".komentar_kriteria($komentare_kriterii['a6'])." {$vety['pk6']} *</h3>";
...
$res.="<h3><span class='odrazka'>07</span>".komentar_kriteria($komentare_kriterii['a7'])." {$vety['pk7']} *</h3>";
...
{  
  ...
  $res.='<br /><input type="submit" value="Uložit" />';
  ...
  else  $res.="<span onclick=\"$(window).scrollTop(parseInt($('#dotaznik').position().top-30));$('#dotaznik').tabs( 'option', 'active' , 1 )\" class='nextStep'>Přejít na další krok -> VOLITELNÉ POŽADAVKY</span>";
}  
   ...
   ";
   if(!$materska_skola)//skryte pro MS
   {
   $res.="
    <h3><span class='odrazka'>03</span>".komentar_kriteria($komentare_kriterii['b3'])." {$vety['v16']}</h3>";

    if($b[2])$sel="checked=\"checked\""; else $sel="";$res.="<input type='checkbox' name='b3' id='b3' value='v16' onchange='display_metakriteria_box(this,\"meta_b3\")' $sel />     

    <label for='b3'>Splňujeme</label>";

    if(!empty($metakriteria) and isset($metakriteria['mb3']))$val=$metakriteria['mb3']; else $val="";
    $res.="<div id='meta_b3' ".(!$sel?"style='display:none'":"").">
    <label for='mb3' class='metakriteria'>{$vety['mb3']}</label><br />
    <textarea name='metakriteria[mb3]' id='mb3'>$val</textarea><br />
    </div>";

 

    $res.="<h3><span class='odrazka'>04</span>".komentar_kriteria($komentare_kriterii['b4'])." {$vety['v17']}</h3>";
    ...
    if($b[5])$sel="checked=\"checked\""; else $sel="";$res.="<input type='checkbox' name='b6' id='b6' value='v19' onchange='display_metakriteria_box(this,\"meta_b6\")' $sel />     

    <label for='b6'>Splňujeme</label>";

    if(!empty($metakriteria) and isset($metakriteria['mb6']))$val=$metakriteria['mb6']; else $val="";
    $res.="<div id='meta_b6' ".(!$sel?"style='display:none'":"").">
    <label for='mb6' class='metakriteria'>{$vety['mb6']}</label><br />
    <textarea name='metakriteria[mb6]' id='mb6'>$val</textarea><br />
    </div>";
    ...
    if($b[7])$sel="checked=\"checked\""; else $sel="";$res.="<input type='checkbox' name='b8' id='b8' value='v21' onchange='display_metakriteria_box(this,\"meta_b8\")' $sel />

    <label for='b8'>Splňujeme</label>

    ";

    if(!empty($metakriteria) and isset($metakriteria['mb8']))$val=$metakriteria['mb8']; else $val="";
    $res.="<div id='meta_b8' ".(!$sel?"style='display:none'":"").">
    <label for='mb8' class='metakriteria'>{$vety['mb8']}</label><br />
    <textarea name='metakriteria[mb8]' id='mb8'>$val</textarea><br />
    </div>";
    ...
    if($b[11])$sel="checked=\"checked\""; else $sel="";$res.="<input type='checkbox' name='b12' id='b12' value='v25' onchange='display_metakriteria_box(this,\"meta_b12\")' $sel />     

    <label for='b12'>Splňujeme</label>";

    if(!empty($metakriteria) and isset($metakriteria['mb12']))$val=$metakriteria['mb12']; else $val="";
    $res.="<div id='meta_b12' ".(!$sel?"style='display:none'":"").">
    <label for='mb12' class='metakriteria'>{$vety['mb12']}</label><br />
    <textarea name='metakriteria[mb12]' id='mb12'>$val</textarea><br />
    </div>";

    $res.="
    <h3><span class='odrazka'>13</span>".komentar_kriteria($komentare_kriterii['b13'])." {$vety['v26']}</h3>";

    if($b[12])$sel="checked=\"checked\""; else $sel="";$res.="<input type='checkbox' name='b13' id='b13' value='v26' onchange='display_metakriteria_box(this,\"meta_b13\")' $sel />     

    <label for='b13'>Splňujeme</label>";

    if(!empty($metakriteria) and isset($metakriteria['mb13']))$val=$metakriteria['mb13']; else $val="";
    $res.="<div id='meta_b13' ".(!$sel?"style='display:none'":"").">
    <label for='mb13' class='metakriteria'>{$vety['mb13']}</label><br />
    <textarea name='metakriteria[mb13]' id='mb13'>$val</textarea><br />
    </div>";
...
{  
  ...
  $res.="<span onclick=\"$(window).scrollTop(parseInt($('#dotaznik').position().top-30));$('#dotaznik').tabs( 'option', 'active' , 0 )\" class='previousStep'>Vrátit se o krok zpět</span><span onclick=\"$(window).scrollTop(parseInt($('#dotaznik').position().top-30));$('#dotaznik').tabs(  'option', 'active' , 2 )\" class='nextStep'>Přejít na další krok -> Identifikační údaje</span>";
}
...
$res.="</select><br />
      ...
      <input type='text' name='web' id='web' value='{$data['skola'][7]}' /><br />
       ...
       {
         $res.="
        <label for='extratrida_nazev'>Název projektu EXTRA TŘÍDA</label><br />
        <input type='text' name='extratrida_nazev' id='extratrida_nazev' value='{$data['skola'][32]}' /><br />
        <label for='extratrida_url'>Odkaz na web projektu EXTRA TŘÍDA</label><br />
        <input type='text' name='extratrida_url' id='extratrida_url' value='{$data['skola'][31]}' /><br />
         ";
       }
        else $res.="<input type='hidden' name='extratrida_nazev' value='' /><input type='hidden' name='extratrida_url' value='' />";
      ...
      <span class='info'>";
      if($materska_skola)
      $res.="(1 sada je v ceně základního poplatku 500 Kč. V případě, že máte více budov, na které chcete umístit označení Rodiče vítáni, vám můžeme zaslat další sadu. Druhá sada je za doplatek 80 Kč, tj. celkem 580 Kč. Třetí sada za 40 Kč, celkem 620 Kč.) Samolepka může být umístěna pouze na budově mateřské školy).";
      else $res.="(1 sada je v ceně základního poplatku 500 Kč. V případě, že máte více budov, na které chcete umístit označení Rodiče vítáni, vám můžeme zaslat další sadu.  Druhá sada je za doplatek 80 Kč, tj. celkem  580 Kč. Třetí sada za 40 Kč, celkem 620 Kč.)  Samolepka může být umístěna pouze na budově základní školy či víceletého gymnázia (ne však např. na budově mateřské školy, která patří k základní škole).";
      ...
      <span class='info'> ";
      //if($materska_skola)$res.="(certifikát a cedule ve stejném provedení) - 1 sada za poplatek 250 Kč.";
      //else 
      $res.="V případě Vašeho zájmu dodáme dodatečně - individuální jednání";
      $res.="</span><br />
      ...
      <textarea name='kde_se_dozvedel' id='kde_se_dozvedel' cols='80' rows='8'>{$data['skola'][22]}</textarea><br />";
        ...
        $res.="<input type=\"hidden\" name=\"materska_skola\" value=\"$materska_skola\" />";
...
{  
  ...
  else $res.="<span onclick=\"$(window).scrollTop(parseInt($('#dotaznik').position().top-30));$('#dotaznik').tabs(  'option', 'active' , 1 )\" class='previousStep'>Vrátit se o krok zpět</span><span onclick=\"$(window).scrollTop(parseInt($('#dotaznik').position().top-30));  $('#dotaznik').tabs(  'option', 'active' , 3 );\" class='nextStep'>Přejít na další krok -> Odeslání dotazníku</span>";
}
 ...
 $('#jmeno_zastupce_ro').css({'border':'1px solid #dcdcdc'});
 $('#email_zastupce_ro').css({'border':'1px solid #dcdcdc'});
 $('#telefon_zastupce_ro').css({'border':'1px solid #dcdcdc'});
   
if($('#a22').is(':checked') && $('#a23').val()==''){alert('Je nutné vyplnit odkaz na web školy, kde lze najít kontakty');$('#a23').css({'border':'1px solid red'});$('#dotaznik').tabs( 'option', 'selected',0);return false;}   
if($('#b3').is(':checked') && $('#mb3').val()==''){alert('Je nutné vyplnit vyplnit odkaz na web školy, kde lze najít bližší informace, nebo stručně vypište, jaká je praxe, pokud nejsou konzultační hodiny uvedeny na webu.');$('#mb3').css({'border':'1px solid red'});$('#dotaznik').tabs( 'option', 'selected',1);return false;}   
if($('#b6').is(':checked') && $('#mb6').val()==''){alert('Je nutné vyplnit odkaz na web školy, kde lze najít ŠVP či jeho souhrn.');$('#mb6').css({'border':'1px solid red'});$('#dotaznik').tabs( 'option', 'selected',1);return false;}   
if($('#b8').is(':checked') && $('#mb8').val()==''){alert('Je nutné stručně uvést stručně, jakým způsobem rodiče na vaší škole mohou ovlivňovat stravování dětí.');$('#mb8').css({'border':'1px solid red'});$('#dotaznik').tabs( 'option', 'selected',1);return false;}   
if($('#b12').is(':checked') && $('#mb12').val()==''){alert('Je nutné vyplnit odkaz na web školy, kde lze najít bližší informace.');$('#mb12').css({'border':'1px solid red'});$('#dotaznik').tabs( 'option', 'selected',1);return false;}   
if($('#b13').is(':checked') && $('#mb13').val()==''){alert('Je nutné vyplnit odkaz na diskuzní fórum školy.');$('#mb13').css({'border':'1px solid red'});$('#dotaznik').tabs( 'option', 'selected',1);return false;}   

   if(!$('#nazev').attr('value')){alert('Je nutné vyplnit název školy');$('#nazev').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;} 

   if(!$('#adresa').attr('value')){alert('Je nutné vyplnit adresu školy');$('#adresa').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}

   if(!$('#icSkoly').attr('value')){alert('Je nutné vyplnit identifikační číslo školy');$('#icSkoly').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}

   //if(!$('#kontaktniOsoba').attr('value')){alert('Je nutné vyplnit kontaktní osobu');$('#kontaktniOsoba').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}

   if(!$('#telefon').attr('value')){alert('Je nutné vyplnit telefon školy');$('#telefon').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}

   if(!$('#email').attr('value')){alert('Je nutné vyplnit email školy, který bude uveden v profilu');$('#email').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}

   //if(!$('#email2').attr('value')){alert('Je nutné vyplnit email školy sloužící ke komunikaci ohledně faktury, PR apod.');$('#email2').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}

   if(!$('#jmeno_reditele').attr('value')){alert('Je nutné vyplnit jméno ředitele/ředitelky, které bude uvedeno v profilu');$('#jmeno_reditele').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}
   if(!$('#email_reditel').attr('value')){alert('Je nutné vyplnit email ředitele/ředitelky');$('#email_reditel').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}
   if(!$('#telefon_reditel').attr('value')){alert('Je nutné vyplnit telefon na ředitele/ředitelku');$('#telefon_reditel').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}
   
   if(!$('#jmeno_zastupce_ro').attr('value')){alert('Je nutné vyplnit jméno zástupce rodičovské organizace');$('#jmeno_zastupce_ro').css({'border':'1px solid red'});$('#dotaznik').tabs({selected:2});return false;}
   if(!$('#telefon_zastupce_ro').attr('value')){alert('Je nutné vyplnit telefon zástupce rodičovské organizace');$('#telefon_zastupce_ro').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}
   if(!$('#email_zastupce_ro').attr('value')){alert('Je nutné vyplnit e-mail zástupce rodičovské organizace');$('#email_zastupce_ro').css({'border':'1px solid red'});$('#dotaznik').tabs(  'option', 'selected',2);return false;}
   
   if(!$('#souhlas').attr('checked')){alert('Je nutné udělit souhlas s poskytnutím osobních údajů');$('#dotaznik').tabs(  'option', 'selected',3);return false;}
   if($('#kontrola').val()!='modrá' && $('#kontrola').val()!='modra'){alert('Kontrolní otázka je chybně zodpovězená ');$('#dotaznik').tabs(  'option', 'selected',3);return false;}

   if($('#popis').val().length>800){alert('Popis je příliš dlouhý - '+$('#popis').val().length+' znaků. Je nutné dodržet maximální délku 800 znaků včetně mezer.');$('#dotaznik').tabs(  'option', 'selected',0);return false;}

   

   \"/></p>

   <span onclick=\"$(window).scrollTop(parseInt($('#dotaznik').position().top-30));$('#dotaznik').tabs(  'option', 'active' , 2 )\" class='previousStep'>Vrátit se o krok zpět</span></div>";
...
$sql="SELECT MAX(cislo) FROM skoly_meta WHERE cislo <10000";
$cislo=db_dotaz($sql);
$res.="<p>Poslední číslo certifikátu je <span style='color:red;'>\"".$cislo[0][0]."\"</span></p>";
$res.=spravaSkolAdminDetail($_GET['skola']);
}
else
...
if($_POST['akce']=='vyzvatSkoluKRecertifikaci')
{
vyzvat_skolu_k_recertifikaci($_POST['skola']);
...
db_dotaz($sql);
$zprava="Škola byla přesunuta do Zveřejněných.";
}
...
db_dotaz($sql);
$zprava="Škola byla přesunuta do Necertifikujeme.";
}
...
db_dotaz($sql);
$zprava="Škola byla přesunuta do Rozpracovaných.";
}

if($_POST['akce']=='doProslych')
{
$sql="UPDATE skoly SET faze='proslaCertifikace' WHERE id='{$_POST['skola']}'";
db_dotaz($sql);
$zprava="Škola byla přesunuta do Prošlých.";
}
...
foreach ($_FILES["fotka"]["error"] as $key => $error) {
 ...
 $zprava="Fotografie byla nahrána.";
}
...
case 'recertifikaceKeSchvaleni':{$res="<h2>Správa škol - recertifikace čekající na kontrolu a schválení správcem</h2>&nbsp;&nbsp;&nbsp;<a href='/wp-content/plugins/export-excel/excel.php?faze=kompletni'>Export pro Excel - kompletní</a>";break;}

}
if($zprava)$res.='<div class="message">'.$zprava.'</div>';
...
<tr><th></th><th>Číslo</th><th>Typ certifikace</th><th>Fakt. zaplacena</th><th class='nazev'>Název školy</th><th>Adresa<th>IČ</th><th>Email (profil)</th><th>Email (kontaktní osoba)</th><th>Email (rodičovská organizace)</th><th>Certifikována</th><th>Rok certifikace</th><th>Zveřejněna</th><th>Datum zveřejnění<br />(certifikace)</th><th>1.Email</th><th>Fakt. odesl.</th><th>Obálka odesl.</th><th class='pozn'>Pozn.</th><th>Počet sad</th><th>Cedule angl.</th><th>Kniha</th><th>Akce</th></tr>
...
";
  ...
  if($spravaSkol=='spravaSkol5'){
   $akce.="<form method=\"post\" action=\"?page=$spravaSkol\"><input type=\"hidden\" name='skola' value=\"{$s[3]}\" /><input type=\"hidden\" name='akce' value=\"doZpracovanych\" /><input type=\"submit\" value=\"Do zveřejněných\" /></form>";
   $akce.="<form method=\"post\" action=\"?page=$spravaSkol\"><input type=\"hidden\" name='skola' value=\"{$s[3]}\" /><input type=\"hidden\" name='akce' value=\"doProslych\" /><input type=\"submit\" value=\"Do prošlých\" /></form>";
 }
  ...
  if($s[1])$datum=date('d.m.Y',strtotime($s[2]));else $datum='';
    ...
    { 
     $sql="SELECT COUNT(*) FROM skoly WHERE id='".$s[3]."' AND certifikovana!='-1'"; //aktualni zaznam skoly, ale pouze v pripade, ze si obnovili certifikaci
     $rokc=db_dotaz($sql);
     $sql="SELECT COUNT(*) FROM skoly_archiv WHERE id='".$s[3]."'";//zaznamy z predeslych let
     $rokc2=db_dotaz($sql);
     
     $rok_certifikace=$rokc[0][0]+$rokc2[0][0];
    }
    
    $typ_certifikace=$s[23]?'MŠ':"ZŠ";
    //print_r($s);exit;
    //$zaplaceno=' style="background:red;color:white;"';else $zaplaceno='';
    $res.= "<tr$cl><td><form action=''><input type='checkbox' name='hromadnaAkce{$s[3]}' class='hromadnaAkce' value='{$s[3]}' /></form></td><td>{$s[13]}</td><td>$typ_certifikace</td><td>$zaplaceno</td><td><a href=\"?page=$spravaSkol&amp;skola=".$s[3]."\">".$s[0]."</a></td><td>{$s[14]}</td><td>{$s[15]}</td><td>{$s[16]}</td><td>{$s[21]}</td><td>{$s[22]}</td><td>$certifikovana"; if($s[4]==-1)$res.="<br /><form method='post' action=''><input type='hidden' name='akce' value='znovuOdeslatMailUpozorneni' /><input type='hidden' name='skola' value='{$s[3]}' /><input type='submit' value='Odeslat upozornění' /></form>";if($s[4]==1 && $faze=="zpracovana")$res.="<br /><form method='post' action=''><input type='hidden' name='akce' value='vyzvatSkoluKRecertifikaci' /><input type='hidden' name='skola' value='{$s[3]}' /><input type='submit' value='Vyzvat k recertifikaci' onclick=\"return confirm('Skutečně chcete tuto školu vyzvat k recertifikaci před vypršením termínu?');\"/></form>";$res.="</td><td>$rok_certifikace</td><td>".$zverejneno."</td><td>".$datum."</td>
...
{
$skola=$_GET['skola'];
if(is_numeric($skola) and $skola>0)
{
$sql="SELECT certifikovana FROM skoly WHERE id='$skola'";
$s=db_dotaz($sql);
//echo hashSkoly($skola);
if(($s[0][0]==-1 or $_GET['adminPreview']=='true') and hashSkoly($skola)==$_GET['h']){

$sql="SELECT varianta FROM odpovedi_dotaznik WHERE id_skoly='$skola'";
global $wpdb;
$result = $wpdb->get_row( $sql, OBJECT );

if(AKTUALNI_VARIANTA != $result->varianta)
{
  $sql="UPDATE odpovedi_dotaznik SET varianta='".AKTUALNI_VARIANTA."',`a1`='', `a2`='', `a3`='', `a4`='', `a5`='', `a6`='', `a7`='', `b1`='', `b2`='', `b3`='', `b4`='', `b5`='', `b6`='', `b7`='', `b8`='', `b9`='', `b10`='', `b11`='', `b12`='', `b13`='', `b14`='', `b15`='', `b16`='', `b17`='', `ma22`='', `mb3`='', `mb6`='', `mb8`='', `mb12`='', `mb13`='' WHERE id_skoly='$skola'";
  db_dotaz($sql);//zmenit variantu dotazniku na novou a vymazat predchozi odpovedi
}
...
elseif(get_the_ID()==3182)$content="Neplatná adresa stránky!";
//else $content="Neplatná adresa stránky!";
return $content;
}
...
$sql="SELECT ma22,mb3,mb6,mb8,mb12,mb13 FROM odpovedi_dotaznik$archiv WHERE id_skoly='$skola' $rok";
$metakriteria=$wpdb->get_results($sql,ARRAY_A);
//print_r($metakriteria);echo $sql;exit;
if(is_array($metakriteria))
foreach ($metakriteria as $krit)
{
$data['metakriteria']=$krit;
}

$sql="SELECT datum_certifikace,prvni_email,faktura_odeslana,faktura_zaplacena,obalka_odeslana,poznamka_meta,naplnovani_kriteria,tipy_na_plneni,cislo FROM skoly_meta$archiv WHERE id_skoly='$skola' $rok";
...
#dotaznik input[type=\"text\"],#dotaznik textarea, #dotaznik select{width:600px;}
  .wp-admin #dotaznik h3{font-size:1.1em;}
...
div.message{ background: #B8F49F; border: 1px solid green; margin: 10px 0;padding: 5px;}
label.metakriteria{color:green;margin-top:5px;}
</style>";
...
//echo "<script type='text/js' src='/wp-content/themes/rodicevitanicz/jquery.ui.datepicker.js'></script>";
}
...
//pokud se zmenila varianta, vyprazdnit ulozene odpovedi a nechat je pri recertifikaci vsechna hlavni a vedlejsi kriteria vzplnit znovu 
$sql="SELECT varianta FROM odpovedi_dotaznik WHERE id_skoly='$skola'";
global $wpdb;
$result = $wpdb->get_row( $sql, OBJECT );
...
$sql="INSERT INTO skoly_meta (`id_skoly`,`datum_certifikace`,`cislo`) VALUES('{$values[0][0]}','{$values[0][1]}','{$values[0][9]}')";
db_dotaz($sql);
...
if(is_numeric($data))
{
   ...
   $teloMailu = "
<p>Dobrý den,</p>
<p>těší nás, že se Vaše škola cíleně zaměřuje na spolupráci s rodiči a stala se součástí sítě škol nesoucí značku Rodiče vítáni. Budeme rádi, když s námi bude spolupracovat i nadále. Protože <strong>".date("d. m. Y",strtotime(date("Y-m-d", strtotime($skola[3])) . " +1 year"))
...
<p><strong>Od 8.2.2015</strong> začala platit <strong>nová verze kritérií značky Rodiče vítáni pro základní školy</strong>. Zároveň s touto změnou se <strong>program Rodiče vítáni otevírá i mateřským školám</strong>, které je možno také certifikovat značkou Rodiče vítáni - <strong>odlišná kritéria pro MŠ - <a href='http://www.rodicevitani.cz/certifikace-materske-skoly/'>ZDE</a></strong></p>
...
<p>Přečtěte si prosím nová <a href=\"http://www.rodicevitani.cz/wp-content/uploads/2015/02/RV_kriteria_certifikace_ZS_uprava_komentare_2015.pdf\">kritéria a jejich komentáře</a>. Vysvětlujeme v nich, co považujeme za kvalitní naplnění kritéria a co ne. Podle aktuální situace ve škole označte příslušná kritéria a doplňte všechny potřebné informace. <strong>Nezapomeňte vyplnit údaj o zástupci rodičů (jméno, kontakty)</strong> - rodiče jsou partnery v programu a potvrzují škole certifikaci (závazné).</p>
...
} wp_mail('marek.drahovzal@seznam.cz', "Oznámení o výzvě k obnovení certifikace školy {$skola[1]} - {$skola[2]},{$skola[4]}", $teloMailu, $headers);
...
*/
  $teloMailu = "
<p>Dobrý den,</p>
<p>těší nás, že se Vaše škola cíleně zaměřuje na spolupráci s rodiči a stala se součástí sítě škol nesoucí značku Rodiče vítáni. <strong>Certifikace vaší školy včerejším dnem vypršela</strong>, a proto jsme profil vaší školy skryli z mapy aktivních škol. Vaši certifikaci si však snadno můžete obnovit.</p>
<p><strong>Od 8.2.2015</strong> začala platit <strong>nová verze kritérií značky Rodiče vítáni pro základní školy</strong>. Zároveň s touto změnou se <strong>program Rodiče vítáni otevírá i mateřským školám</strong>, které je možno také certifikovat značkou Rodiče vítáni - <strong>odlišná kritéria pro MŠ - <a href='http://www.rodicevitani.cz/certifikace-materske-skoly/'>ZDE</a></strong></p>
<p><strong>Profil vaší školy</strong> naleznete na tomto odkazu <a href='$url'>$url</a>, kde si prosím <strong>zkontrolujte a případně upravte</strong> (doplňte či odeberte)
<br />- základní kritéria, volitelná kritéria,
<br />- kontaktní údaje (ředitel/ka, kontakní osoba, rodičovská organizace),
...
wp_mail('marek.drahovzal@seznam.cz', "Rodiče vítáni - oznámení o ukončení platnosti certifikace {$skola[1]}-{$skola[2]},{$skola[3]}", $teloMailu, $headers);
...
function opravaSouradnic()
{
 $sql="SELECT adresa,id FROM skoly WHERE geoLat=0";
 $data= db_dotaz($sql);
 foreach($data as $d)
 {
$souradnice=generujSouradnice($d[0]);
$sql="UPDATE skoly SET geoLat='{$souradnice['geoLat']}',geoLng='{$souradnice['geoLng']}' WHERE id='{$d[1]}'";
//echo "$sql<br />";
  db_dotaz($sql);
}
}
...
/**
 * nastavi skolu k recertifikaci a posle email bez ohledu na to, kolik skole zbyva dni do konce certifikace
 */ 
function vyzvat_skolu_k_recertifikaci($id_skoly)
{
 $sql="SELECT id,nazev,email2,datumZverejneni,email_reditel FROM skoly JOIN skoly_meta sm ON sm.id_skoly=skoly.id  WHERE faze='zpracovana' AND id='$id_skoly' AND certifikovana='1' LIMIT 1";
   $skoly=db_dotaz($sql);
...
function vyplnenoVeFormulariCertifikace($id)
{

$data=spravaSkolAdminDetail($id,1);
$kraj = $data['skola'][11];
  $res="<br />
      <strong>Oficiální název školy</strong><br />
      {$data['skola'][1]}<br />
    ...
    {$data['skola'][2]}<br />
      ...
      ";
if($kraj=="1")$res.="Jihočeský";
if($kraj=="2")$res.="Jihomoravský";
if($kraj=="3")$res.="Karlovarský";
if($kraj=="4")$res.="Královéhradecký";
if($kraj=="5")$res.="Liberecký";
if($kraj=="6")$res.="Moravskoslezský";
if($kraj=="7")$res.="Olomoucký";
if($kraj=="8")$res.="Pardubický";
if($kraj=="9")$res.="Plzeňský";
if($kraj=="10")$res.="Středočeský";
if($kraj=="11")$res.="Praha";
if($kraj=="12")$res.="Ústecký";
if($kraj=="13")$res.="Vysočina";
if($kraj=="14")$res.="Zlínský";
$res.="<br />";
$res.="
      <strong>IČ školy</strong><br />
      {$data['skola'][4]}<br />

      <strong>Jméno a příjmení ředitele/ředitelky</strong><br />
      {$data['skola'][19]}<br />

      <strong>E-mail ředitele/ředitelky</strong><br />
      {$data['skola'][16]}<br />

      <strong>Telefon na ředitele/ředitelku</strong><br />
      {$data['skola'][17]}<br />

      <strong>Telefon - Profil školy</strong><br />
      {$data['skola'][5]}<br />

      <strong>E-mail - Profil školy</strong><br />
      {$data['skola'][6]}<br />
      
      <strong>Jméno a příjmení kontaktní osoby a její role ve škole - může být shodné se jménem ředitele/ředitelky</strong><br />
      {$data['skola'][3]}<br />

      <strong>E-mail kontaktní osoba - může být shodné s kontaktem na ředitele/ředitelku</strong><br />
      {$data['skola'][15]}<br />

      <strong>Telefon kontaktní osoba - může být shodné s kontaktem na ředitele/ředitelku</strong><br />
      {$data['skola'][18]}<br />

      <strong>Název a IČO rodičovské organizace</strong><br />
      {$data['skola'][27]}<br />
      <strong>Jméno a příjmení zástupce rodičovské organizace</strong><br />
      {$data['skola'][28]}<br />
      <strong>Email zástupce rodičovské organizace</strong><br />
      {$data['skola'][29]}<br />
      <strong>Telefon zástupce rodičovské organizace</strong><br />
      {$data['skola'][30]}<br />
      <strong>Odkaz na web školy</strong><br />
      {$data['skola'][7]}<br />
       ";
        
       $res.="
      <strong>Odkaz na Facebook školy</strong><br />
      {$data['skola'][23]}<br />

      <strong>Popis školy (zaměření, programy)</strong><br />
      {$data['skola'][12]}<br />

      <strong>Počet označení značkou Rodiče vítáni</strong><br />
      {$data['skola'][20]} ks<br />
      <strong>Máme zájem o certifikát a ceduli Rodiče vítáni také v anglické mutaci: </strong><br />
       {$data['skola'][24]}<br />
    
      <strong>Kniha Tomáše Feřtka: Rodiče vítáni: praktický návod, jak usmířit rodiče a učitele našich dětí.</strong><br />
      {$data['skola'][25]} ks<br />
      <strong>Poznámka</strong><br />
      {$data['skola'][21]}<br />

      <strong>Kde jste se o značce Rodiče vítáni dozvěděl/a?</strong><br />
      {$data['skola'][22]}<br /><hr>";
...
/**
 * nacte profily skoly
 * kombinovana skola muze mit dva profily, ZS + MS 
 */ 
function getProfilySkoly($id_skoly)
{
  if(is_numeric($id_skoly))
  {
      $sql="SELECT s2.id,s2.materska_skola FROM skoly s1 JOIN skoly s2 ON s1.ic_skoly=s2.ic_skoly WHERE s1.id='$id_skoly' GROUP BY s2.materska_skola ORDER BY s2.materska_skola";
      $profily = db_dotaz($sql);
      return $profily;
  }
  else return false;
}