...
function SSP_social_buttons() {
	...
	if (isset($_GET['id'])) $full_url .=  "?id=" . $_GET['id'];
  $full_url =  urlencode($full_url);
	...
	if (strlen($SSP_options['heading']) > 1) {
		...
		$buttons .= "<h3 id=\"podnadpisVClanku\">" . $SSP_options['heading'] . "</h3>\n";
    ...
    $sharer = "<a href=\"https://www.facebook.com/sharer/sharer.php?t=" . get_the_title($post->ID) . "&amp;u=$full_url\"><img src='" .   plugins_url(). "/share-center-pro/sdiletFB.jpg' /></a>";
    $buttons .= "<li id=\"sspFacebook\">$sharer</li>\n"; 
	}
...
function ssp_fb_thumbnail() {
	...
	if ($SSP_options['show_facebook'] == 1) {
      ...
      else echo  "<meta property=\"og:image\" content=\"http://rodicevitani.cz/logo.gif\"/>\n";
